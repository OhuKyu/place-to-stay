...
export const register = tryCatch(async (req, res) => {
    ...
    if (existedUser)
        return res
            .status(400)
            .json({ success: false, message: 'Tài khoảng đã tồn tại!' });
...
export const login = tryCatch(async (req, res) => {
    ...
    if (!existedUser)
        return res
            .status(404)
            .json({ success: false, message: 'Tài khoảng chưa tồn tại!' });
    const correctPassword = await bcrypt.compare(password, existedUser.password);
    if (!correctPassword)
        return res
            .status(400)
            .json({ success: false, message: 'Thông tin đăng nhập ko hợp lệ' });
